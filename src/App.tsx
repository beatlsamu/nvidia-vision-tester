...
// ── NARRATOR (accessibility, plain text) ──────────────────────────────────────
...
export default function App() {
  ...
  // — detection / tracking state (original) —
  ...
  // ── tracking result processor (original) ───────────────────────────────────
  ...
  // Detection frame loop (ref-based to avoid stale closures)
  ...
  function handleSelectObject(obj: DetectedObject) {
    setSelectedObject(obj); setTargetLabel(obj.label); setMode('tracking'); setCount(0); setTargetStatus(null); setAlerts([]); prevPresentRef.current = null
  }
  ...
  // Narrator frame loop (always latest via ref)
  ...
  runNarratorLoop.current = async () => {
    ...
    // wait while speaking so narrations don't overlap
  ...
  function startMic() {
    ...
    rec.lang           = 'es-ES' // detects other languages too in practice
    ...
    rec.onresult = (event: SpeechRecognitionEvent) => {
      ...
      if (PAUSE_CMDS.some(c => lower.includes(c))) {
        ...
        // the useEffect will restart the loop
      } else if (NOW_CMDS.some(c => lower.includes(c))) {
        triggerImmediateNarration()
      } else if (transcript.length > 3) {
        // treat any other speech as a question to answer with the camera frame
  ...
  // cleanup when leaving narrator tab
  useEffect(() => {
    if (tab !== 'narrator') {
      stopNarratorCamera()
      stopMic()
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6">
              ...
              <button
                ...
                aria-label={narratorCamActive ? 'Apagar cámara del narrador' : 'Encender cámara del narrador'}
              ...
              <button
                ...
                aria-label={narratorActive ? 'Pausar narración' : 'Iniciar narración continua'}
              ...
              <button
                ...
                aria-label={micActive ? 'Desactivar micrófono' : 'Activar micrófono para preguntas por voz'}
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6 flex flex-col">
              ...
              {narratorText ? (
                <div className="flex-1 flex flex-col">
                  ...
                  <div className={`rounded-2xl p-5 border-2 mb-4 transition-all ${
                    ...
                    <p role="status" aria-live="polite" aria-label="Descripción del entorno"
              ...
              <div className="mt-auto border-t border-zinc-800 pt-4">
                ...
                <p className="text-xs text-zinc-700 mt-3">
                  Nota: el reconocimiento de voz funciona mejor en Chrome y Edge.
